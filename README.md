...
网页会从用户输入中提取主标题、副标题、文案、尺寸与风格；缺少的信息会逐项追问。信息齐全后，服务端通过 GRSai 调用 `gpt-image-2.5-sunburst` 生成两张 2K、`high` 质量的图片。用户可在聊天中预览和下载图片。

输入框支持上传 PNG、JPEG 或 WebP 参考图（最大 4MB）。后续修改会将上一轮生成图作为参考，并保留原尺寸；只有用户明确给出新尺寸时才会改用新尺寸。请确保 `GRSAI_API_KEY` 有足够的图片生成额度。
